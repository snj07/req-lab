...
- `SseApiE2ETest` — generic `GET`/`POST /sse` (`text/event-stream`), finite ping events, POST body echo, `?count=`
...
- `GET /sse` (`?count=`, `?delayMs=`)
- `POST /sse` (echoes a body snippet as the last event)
- `WS /ws`
