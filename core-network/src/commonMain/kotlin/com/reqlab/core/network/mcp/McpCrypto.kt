...
/**
 * SHA-256 digest used for PKCE S256. Implemented in common code so wasm/js/ios
 * don't need a platform crypto actual for hashing.
 */
internal fun sha256(input: ByteArray): ByteArray {
    ...
    for (i in 0 until 8) {
        padded[paddedSize - 1 - i] = ((bitLen ushr (8 * i)) and 0xFF).toByte()
    }
    ...
    fun write(value: Int, index: Int) {
        out[index] = (value ushr 24).toByte()
        out[index + 1] = (value ushr 16).toByte()
        out[index + 2] = (value ushr 8).toByte()
...
expect fun mcpSecureRandomBytes(size: Int): ByteArray

expect val mcpStdioSupported: Boolean

expect fun createStdioTransport(config: com.reqlab.core.model.McpConnectionConfig): McpTransport

expect val mcpInteractiveOAuthSupported: Boolean

expect suspend fun mcpOpenAuthorizeUrlAndAwaitCode(authorizeUrl: String, redirectPort: Int): String
